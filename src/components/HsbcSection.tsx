...
import HSBCLogo from '../logos/HSBC.png';
import { PhoneInput } from 'react-international-phone';
import 'react-international-phone/style.css';

import ShortPointsDisplay from './ShortPointsDisplay';

const HSBCSection: React.FC = () => {
    const [phone, setPhone] = useState("");
    const [response, setResponse] = useState<any>(null);
    const API_URL = process.env.REACT_APP_API_URL;
    const API_KEY = process.env.REACT_APP_HSBC_API_KEY;

    const handleSubmit = async () => {
        try {
            const res = await fetch(`${API_URL}/Company6/api/points_details?phone=${encodeURIComponent(phone)}`, {
                method: 'GET',
                headers: {
    ...
    return (
        <div>
            ...
            <p style={{ color: 'black', maxWidth: '900px', margin: '0 auto', fontFamily: 'Garamond', fontSize: '24px', textAlign: 'center', paddingLeft: '120px', paddingRight: '120px' }}>
                One of the world's largest banking and financial services organizations, providing a comprehensive range of services including personal banking, commercial banking, and wealth management
            </p>
            <div className="mt-10 flex flex-col items-center">
                ...
                <ShortPointsDisplay response={response} value={phone} apiUrl={`${API_URL}`} apiKey={`${API_KEY}`} company={"Company6"} param={"phone"} />
